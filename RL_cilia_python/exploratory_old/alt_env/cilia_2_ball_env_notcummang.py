...
class Cilia2BallEnv(gym.Env):
    ...
    def state_to_angles(self, state):
        ...
        theta[0] = self.bin_to_angle(state[0], 0)
        theta[1] = self.bin_to_angle(state[1], 1)
        return theta
    ...
    def pos_vel_from_two_states(self, state, new_state):
        ...
        theta = self.state_to_angles(mid_state)
        anglevel = state_change * (self.dangle / self.dt)

        X = np.zeros((self.Nballs, 3), dtype=float)
        U = np.zeros((self.Nballs, 3), dtype=float)

        X[0] = self.X0 + self.len * np.array(
            [np.sin(theta[0]), 0.0, np.cos(theta[0])], dtype=float
        )
        X[1] = X[0] + self.len * np.array(
            [np.sin(theta[1]), 0.0, np.cos(theta[1])], dtype=float
        )

        U[0] = self.len * anglevel[0] * np.array(
            [np.cos(theta[0]), 0.0, -np.sin(theta[0])], dtype=float
        )
        U[1] = U[0] + self.len * anglevel[1] * np.array(
            [np.cos(theta[1]), 0.0, -np.sin(theta[1])], dtype=float
        )
    ...
    def clip_state(self, state):
        state = np.asarray(state, dtype=int)
        clipped = np.minimum(np.maximum(state, np.zeros(2, dtype=int)), self.n_bins - 1)
        return clipped
    ...
    def step(self, action):
        ...
        if self.precompute and self.flux_table is not None and self.next_state_table is not None:
            i, j = self.state
    ...
    def render(self):
        theta1 = self.bin_to_angle(self.state[0], 0)
        theta2 = self.bin_to_angle(self.state[1], 1)
        print(
            f"state={self.state}, angles=({theta1:.6f}, {theta2:.6f}), "
