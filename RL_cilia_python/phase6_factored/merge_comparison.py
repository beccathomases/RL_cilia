...
FACTORED_SEARCH_ROOTS = [
    os.path.join(BASE, "phase6_factored"),
    os.path.join(BASE, "phase5_5ball"),
    os.path.join(BASE, "phase4_Nball_exploration"),
    ".",
]
...
def load_factored():
    if MONO_NEW_CSV:  # not factored; placeholder for symmetry if needed
        pass
    found = []
    seen = set()
    for root in FACTORED_SEARCH_ROOTS:
        for path in glob.glob(os.path.join(root, "**", "summary*.csv"), recursive=True):
            ...
            if "action_mode" in df.columns:
                fac = df[df["action_mode"].astype(str) == "factored"]
                if len(fac):
                    found.append((path, fac))
    if not found:
        return None, []
    frames = [standardize(f, "factored") for _, f in found]
    combined = pd.concat(frames, ignore_index=True)
    # per-case summary.csv and the root summary_all.csv overlap; keep one row
    # per (N, dtheta, seed) so seeds aren't counted twice.
    combined = combined.drop_duplicates(
        subset=["nballs", "dtheta_label", "seed"]).reset_index(drop=True)
