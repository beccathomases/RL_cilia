...
Visualizer for Phase 5 PPO N=5 result.npz files.
...
def chain_positions_from_phi(phi, total_length=1.0):
    """
    phi = relative joint angles, shape (N,)
    psi = cumulative segment angles.
    Segment length = total_length/N.
    """
...
def main():
    ...
    parser.add_argument(
        "--result",
        default="results/ppo_sweeps_n5/N5_dtheta_pi20/seed_000/result.npz",
        help="path to result.npz",
    )
    parser.add_argument(
        "--outdir",
        default=None,
        help="output directory; default is figures/<seed folder>",
    )
    ...
    if args.outdir is None:
        outdir = Path("figures") / "ppo_sweeps_n5" / case_name / seed_name
    else:
    ...
    plot_stroke_overlay(
        ...
        "Learned N=5 beat: stroke overlay\n" + tag,
    )
