...
Phase 5 baseline: N=5 PPO sweep.

This is the first no-Howard scale-up test. It uses the same generalized
N-ball environment, but does not require building the full transition/reward
table.

It trains PPO, rolls out the deterministic policy, detects a cycle, and records
Howard-free diagnostics:
    ...
    wrong_orientation_flag

Outputs:
    results/ppo_sweeps_n5/N5_dtheta_pi20/seed_000/model.zip
    results/ppo_sweeps_n5/N5_dtheta_pi20/seed_000/result.npz
    results/ppo_sweeps_n5/N5_dtheta_pi20/seed_000/summary.json
    results/ppo_sweeps_n5/N5_dtheta_pi20/summary.csv
    results/ppo_sweeps_n5/summary_all.csv

Example smoke test:
    caffeinate -i python ppo_sweep_n5.py \
      --cases pi20 \
      --seeds 0 1 \
      --timesteps 300000 \
      --force
...
def make_raw_env(nballs: int, dtheta: float, seed: int | None = None):
    ...
    """
    attempts = [
        dict(Nballs=nballs, dtheta=dtheta, precompute=False, reset_mode="midpoint"),
        dict(Nballs=nballs, dtheta=dtheta, precompute=False, reset_mode="fixed"),
        dict(Nballs=nballs, dtheta=dtheta, precompute=False, seed=seed),
        dict(Nballs=nballs, dtheta=dtheta, precompute=False),
        dict(nballs=nballs, dtheta=dtheta, precompute=False),
        dict(N=nballs, dtheta=dtheta, precompute=False),
        dict(n_balls=nballs, dtheta=dtheta, precompute=False),
    ]
...
def make_train_env(nballs: int, dtheta: float, seed: int):
    env = make_raw_env(nballs=nballs, dtheta=dtheta, seed=seed)
    env = Monitor(env)
    return env
...
def train_one(
    ...
    print("=" * 72)
    print(f"[train] N={nballs}, dtheta={dtheta_label}, seed={seed}, timesteps={timesteps}")
    print(f"[out]   {seed_dir}")

    train_env = make_train_env(nballs=nballs, dtheta=dtheta, seed=seed)
    ...
    eval_env = make_raw_env(nballs=nballs, dtheta=dtheta, seed=seed)
...
def parse_args():
    ...
    p.add_argument("--outroot", type=str, default="results/ppo_sweeps_n5")
...
def main():
    ...
    for case in args.cases:
        ...
        case_dir = root / f"N{args.nballs}_dtheta_{case}"
        case_dir.mkdir(parents=True, exist_ok=True)

        print("=" * 72)
        print(f"[case] N={args.nballs}, {case}: dtheta={dtheta}")
        print("=" * 72)
    ...
    print("\n[done] Phase 5 N=5 PPO sweep complete.")
