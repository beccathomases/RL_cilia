...
class QNet(nn.Module):
    def __init__(self, input_dim=3, hidden=64, n_actions=26):
        super().__init__()
...
def extract_policy(q_net, env, device="cpu"):
    ...
    for i in range(n0):
        for j in range(n1):
            for k in range(n2):
                s = np.array([i, j, k], dtype=np.float32) / scale
                with torch.no_grad():
                    inp = torch.tensor(s, dtype=torch.float32, device=device).unsqueeze(0)
                    q_vals = q_net(inp)
                policy[i, j, k] = int(torch.argmax(q_vals, dim=1).item())
...
def train_dqn(
    ...
    q_net = QNet(input_dim=3, hidden=64, n_actions=env.action_space.n).to(device)
    target_net = QNet(input_dim=3, hidden=64, n_actions=env.action_space.n).to(device)
    ...
    for ep in range(episodes):
        ...
        with torch.no_grad():
            sample = torch.zeros((1, 3), dtype=torch.float32, device=device)
...
if __name__ == "__main__":
    ...
    # conservative first experiment: just a few seeds
    seeds = [0, 1, 2]
    ...
    for seed in seeds:
        run_name = f"dqn_3ball_bins11x21x21_ep{episodes}_steps1500_lr{lr:.0e}_seed{seed}"
        print("\n" + "=" * 72)
