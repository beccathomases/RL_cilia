...
def train_ppo(env_settings, total_timesteps, seed, ppo_settings, device="cpu"):
    random.seed(seed)
    np.random.seed(seed)
...
if __name__ == "__main__":
    outdir = "ppo_3ball_tuning_runs_round2"
    ...
    # 4 configs x 3 seeds = 12 runs
    configs = [
        {"n_steps": 2048, "learning_rate": 3e-4, "ent_coef": 0.0},
        {"n_steps": 2048, "learning_rate": 1e-4, "ent_coef": 0.0},
        {"n_steps": 1024, "learning_rate": 3e-4, "ent_coef": 0.0},
        {"n_steps": 2048, "learning_rate": 3e-4, "ent_coef": 0.001},
    ]
    ...
    for cfg in configs:
        for seed in seeds:
            ...
            run_name = (
                f"ppo_3ball_steps{total_timesteps}_seed{seed}"
                f"_ns{n_steps}_lr{lr:.0e}_ent{ent_coef:g}"
            )
