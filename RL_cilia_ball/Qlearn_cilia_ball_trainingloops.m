...
% will decay like alpha0*(.99)^(episodes-1)
epsilon0 = 0.5; % initial exploration vs exploitation
% change to 0.2 and 0.1 (Charlotte)
% will decay like alpha0*(.99)^(episodes-1)
...
for seeds = 1:10
    ...
    for ep = 1:nEpisodes
        s = env.reset();
        G = 0;
        epsilon = epsilon0*(.99)^(ep-1);  % geometric decay for epsilon
        alpha = alpha0*(.99)^(ep-1);      % geometric decay for alpha
        for t = 1:maxSteps
            % epsilon-greedy
            if rand < epsilon
