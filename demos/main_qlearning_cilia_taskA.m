...
% Regularization: penalize bending energy ∫ kappa(s)^2 ds
%
...
for ep = 1:N_EPISODES
    ...
    % Track best FEASIBLE by span (main objective)
    if infoFinal.minY >= params.terminate_minY
    ...
    % (Optional) track best FEASIBLE by one-step reward
    if infoFinal.minY >= 1e-6
        if rFinal > bestR
            bestR = rFinal;
            bestC_byR = c;
        end
