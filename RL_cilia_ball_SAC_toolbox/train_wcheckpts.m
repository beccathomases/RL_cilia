% train_cilia_SAC_agent_closure_transport_checkpointed.m
%
% OLD closure+transport setup + checkpointed online training
%
% This version:
%   - keeps the OLD closure+transport reward
%   - keeps the OLD mixed_demo reset setup
%   - does NOT use the seed-3 reset-pool mix
%   - saves online-training checkpoints every chunk
%
% Change only "seedin" to try a different seed.
...
onlineEpisodesTotal = 400;     % try 400 first
maxStepsPerEpisode  = 250;     % old good regime
episodesPerChunk    = 50;      % checkpoint every 50 episodes
nChunks = onlineEpisodesTotal / episodesPerChunk;

if abs(nChunks - round(nChunks)) > 1e-12
    error('onlineEpisodesTotal must be divisible by episodesPerChunk.');
end
...
% OLD mixed demo reset settings
pDemoReset = 0.7;
demoNoiseScaleFactor = 0.25;
...
% ------------------------------------------------------------
% Reward settings  (OLD closure+transport version)
% ------------------------------------------------------------
stepPenalty      = 0.0010;
...
% build base name early so checkpoints can use it
onlineEpisodes = onlineEpisodesTotal;

baseName = sprintf("csac_closureTransport_sp%03d_fs%03d_lcl%03d_ltr%03d_sig%03d_arm%02d_cl%02d_fx%02d_lcy%03d_s%03d_e%d_t%d_ss%02d", ...
    ...
    seed, onlineEpisodes, maxStepsPerEpisode, ...
...
% ============================================================
% BUILD DEMONSTRATION BUFFER AND DEMO RESET STATES
% ============================================================
...
if isempty(demoPhiList)
    error('No demoPhiList states were built.');
end

fprintf('Built %d demonstration experiences.\n', numel(expBatch));
fprintf('Built demoPhiList with %d phase points.\n', size(demoPhiList,2));
